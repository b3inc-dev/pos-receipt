...
| 項目 | GAS | POS（アプリ） |
|------|-----|----------------|
| 集計に使う注文 | **created_at がその日** の注文 ＋ **updated_at がその日** の注文（`fetchAllOrdersSmart` でユニオン） | **created_at がその日** の注文のみ |
| 補足 | 「その日に更新された注文」（例: 翌日キャプチャ・その日の返金）も集計対象に含める | updated_at で取る注文は「返金オーバーレイ」用の別クエリで、**返金分だけ**を追加。売上（total）には使わない |

**影響**  
- 前日作成・当日キャプチャ（または当日更新で売上追加）した注文は、GAS ではその日の売上に含まれるが、POS では **total に含まれない** ため、**売上が少なく出る**可能性があります。
